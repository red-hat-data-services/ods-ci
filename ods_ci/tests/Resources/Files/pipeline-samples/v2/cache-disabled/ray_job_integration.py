...
def ray_fn(
    ...
    with open(mnist_directory + "/mnist_pip_requirements.txt") as pip_requirements_file:
        pip_requirements_file.write(pip_requirements)
    
    assert_jobsubmit_withlogin(self, cluster, mnist_directory)
    
    cluster.down()
    return result

    def assert_jobsubmit_withlogin(self, cluster, mnist_directory):
