...
Container Image Url Using Image Digest Instead Of Tags Based on Project Name
    ...
    FOR    ${namespace}    IN    @{project_list}
           ${return_code}    ${output} =    Run And Return Rc And Output   oc get pods --namespace ${namespace} -o json | grep "\\"image\\"" | cut -d ":" -f2-3 | sort | uniq
           Should Be Equal As Integers	 ${return_code}	 0
           ${isEmpty}    Run Keyword And Return Status    Should Be Empty      ${output}
           IF  ${isEmpty}   Continue For Loop
           ${images} =   Split String    ${output}     ,\n
           FOR    ${image}    IN    @{images}
                  ${status}    Run Keyword And Return Status    Should Contain    ${image}    @sha256
                  IF    not ${status}    Run Keyword And Continue On Failure    Fail      msg=Container image ${image} is not using image digest in namespace ${namespace}
           END
    END
