...
Run Operator e2e tests
    ...
    ${rc}  ${output}=    Run And Return Rc And Output      find /sys/fs/cgroup ; cat /sys/fs/cgroup/cpu.max
    Log To Console    ${output}
    ${rc}  ${output}=    Run And Return Rc And Output      cd ${OPERATOR_GIT_DIR} && time go test -c ./tests/e2e/
    Log To Console    ${output}
