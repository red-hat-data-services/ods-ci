...
Verify Auth Crd
    ...
    END
    Get User Groups From Auth Cr And Check Rolebinding Exists       adminGroups     admingroup-rolebinding
    Get User Groups From Auth Cr And Check Rolebinding Exists       allowedGroups   allowedgroup-rolebinding
...
Get User Groups From Auth Cr And Check Rolebinding Exists
    ...
    FOR    ${user}    IN    @{groups_list}
        ...
        ...    oc get RoleBinding ${rolebinding} -n redhat-ods-applications -o jsonpath='{.subjects[?(@.name==${user})]}'   #robocop: disable:line-too-long
        Should Be Equal As Integers    ${rc}    0
    END
