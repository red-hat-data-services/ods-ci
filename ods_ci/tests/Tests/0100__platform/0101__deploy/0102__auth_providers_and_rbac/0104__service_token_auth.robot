...
Verify Service Account Can Authenticate Via Token
    ...
    ${token}=    Create Service Account Token    ${TEST_SA_NAME}    ${APPLICATIONS_NAMESPACE}    duration=10m
    ...
    ...    expected_user=system:serviceaccount:${APPLICATIONS_NAMESPACE}:${TEST_SA_NAME}
...
Verify Service Account Token Has User Identity
    ...
    Should Contain    ${response}    system:serviceaccount:${APPLICATIONS_NAMESPACE}:${TEST_SA_NAME}
...
Verify No Token Results In Auth Challenge
    ...
    ${response}=    Call Gateway Without Token    path=/echo
...
Service Token Auth Suite Setup
    ...
    Deploy Echo Service    namespace=${APPLICATIONS_NAMESPACE}
    Create Test Service Account    namespace=${APPLICATIONS_NAMESPACE}
    Create HTTPRoute For Echo Service    namespace=${APPLICATIONS_NAMESPACE}
...
Service Token Auth Suite Teardown
    ...
    Remove Echo Service    namespace=${APPLICATIONS_NAMESPACE}
