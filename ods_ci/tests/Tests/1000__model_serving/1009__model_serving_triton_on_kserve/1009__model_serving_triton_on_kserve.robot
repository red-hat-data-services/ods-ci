...
Test Onnx Model gRPC Inference Via UI (Triton on Kserve)
    ...
    ...    namespace=${PRJ_TITLE}
    ${EXPECTED_INFERENCE_GRPC_OUTPUT_ONNX}=     Load Json File     file_path=${EXPECTED_INFERENCE_GRPC_OUTPUT_FILE}     double_quotes=$TRUE
    Log    ${EXPECTED_INFERENCE_GRPC_OUTPUT_ONNX}
    Run Keyword And Continue On Failure    Verify Model Inference With Retries
    ...    ${ONNX_MODEL_NAME}    ${INFERENCE_GRPC_INPUT_ONNX}    ${EXPECTED_INFERENCE_GRPC_OUTPUT_ONNX}    token_auth=${FALSE}
    ...    project_title=${PRJ_TITLE}       service_port=443
