...
${EXPECTED_INFERENCE_OUTPUT_PYTHON}=       {"model_name":"python__isvc-ff65c777d4","model_version":"1","outputs":[{"name":"OUTPUT0","datatype":"FP32","shape":[4],"data":[0.921443,0.6223347,0.80593854,1.2578542]},{"name":"OUTPUT1","datatype":"FP32","shape":[4],"data":[0.49091366,-0.027157962,-0.5641784,0.690631]}]}
${INFERENCE_INPUT_PYTHON}=       @tests/Resources/Files/triton/modelmesh-triton-python-input.json
${KSERVE_MODE}=    ModelMesh
${PROTOCOL}=     http
${TEST_NS}=        tritonmodel
...
${MODELMESH_RUNTIME_NAME}=     triton-modelmesh-runtime

*** Test Cases ***
Test Python Model Rest Inference Via API (Triton on Modelmesh)    # robocop: off=too-long-test-case
    [Documentation]    Test the deployment of python model in Modelmesh using Triton
    [Tags]    Tier2    RHOAIENG-16920    RunThisTest
    Setup Test Variables    model_name=${PYTHON_MODEL_NAME}    use_pvc=${FALSE}    use_gpu=${FALSE}
    ...    kserve_mode=${KSERVE_MODE}   model_path=triton/model_repository/python/
    Set Project And Runtime    runtime=${MODELMESH_RUNTIME_NAME}     protocol=${PROTOCOL}     namespace=${test_namespace}
    ...
    ...    model_format=python  serving_runtime=${MODELMESH_RUNTIME_NAME}
    ...    version="1"
    ...    limits_dict=${limits}    requests_dict=${requests}    kserve_mode=${KSERVE_MODE}
    Deploy Model Via CLI    isvc_filepath=${INFERENCESERVICE_FILLED_FILEPATH}
    ...    namespace=${test_namespace}
    Log     ${INFERENCESERVICE_FILLED_FILEPATH}
    ...
    END
    Verify Model Inference With Retries   model_name=${PYTHON_MODEL_NAME}    inference_input=${INFERENCE_INPUT_PYTHON}
    ...    expected_inference_output=${EXPECTED_INFERENCE_OUTPUT_PYTHON}   project_title=${test_namespace}
