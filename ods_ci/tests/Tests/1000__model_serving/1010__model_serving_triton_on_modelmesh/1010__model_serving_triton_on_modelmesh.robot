...
Test Keras Model Rest Inference Via UI (Triton on Modelmesh)
    [Documentation]    Test the deployment of an onnx model in Kserve using Triton
    [Tags]    Sanity    RHOAIENG-11564      RunThisTest
    ...
    ...    project_title=${PRJ_TITLE}    timeout=5m
    Verify Model Status    ${PYTORCH_MODEL_NAME}    success
    ${EXPECTED_INFERENCE_REST_OUTPUT_KERAS}=     Load Json File      file_path=${EXPECTED_INFERENCE_KERAS_OUTPUT_FILE}
    ...     as_string=${TRUE}
    Log     ${EXPECTED_INFERENCE_REST_OUTPUT_KERAS}
    Verify Model Inference With Retries    ${PYTORCH_MODEL_NAME}    ${INFERENCE_REST_INPUT_KERAS_FILE}
