...
Cleanup TrustyAI-Tests Setup
    ...
    ...    shell=true    stderr=STDOUT
    IF    ${result.rc} != 0     FAIL   Unable to switch python environment 3.11
...
Run Drift Pytest Framework
    ...
    ${result} =    Run Process  source ${VIRTUAL_ENV_NAME}/bin/activate && cd ${TRUSTYAI-TESTS_DIR} && poetry env use ${PYTHON_VERSION} && poetry install && poetry run pytest -v -s ./trustyai_tests/tests/${TESTS_FOLDER}/ && deactivate     # robocop: disable:line-too-long
