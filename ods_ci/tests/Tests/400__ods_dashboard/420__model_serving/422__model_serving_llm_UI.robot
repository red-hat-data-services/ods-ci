...
#${CAIKIT_ALLTOKENS_ENDPOINT}=    caikit.runtime.Nlp.NlpService/TextGenerationTaskPredict  # grpc - not supported via UI
#${CAIKIT_STREAM_ENDPOINT}=    caikit.runtime.Nlp.NlpService/ServerStreamingTextGenerationTaskPredict  # grpc - not supported via UI
...
Verify User Can Set Requests And Limits For A Model Using The UI
    ...
    ...    namespace=${test_namespace}
    Query Model Multiple Times    endpoint=${CAIKIT_ALLTOKENS_ENDPOINT_HTTP}    model_name=${flan_model_name}    n_times=1
    ...    namespace=${test_namespace}    protocol=http
...
Verify Model Can Be Served And Query On A GPU Node Using The UI
    ...
    [Tags]    Sanity    Tier1    ODS-XXXX   Resources-GPU    ProductBug
...
Verify User Can Serve And Query Flan-t5 Grammar Syntax Corrector Using The UI
    ...
    Sleep    30s
    Query Model Multiple Times    endpoint=${CAIKIT_ALLTOKENS_ENDPOINT_HTTP}    model_name=${flan_model_name}    n_times=1
    ...    namespace=${test_namespace}    query_idx=${1}    protocol=http
    Query Model Multiple Times    model_name=${flan_model_name}
...
Verify User Can Access Model Metrics From UWM Using The UI
...
Query Model Multiple Times
    ...
    ELSE IF    "${protocol}" == "http"
        ${body}=    Set Variable    {"model_id": "${model_name}","inputs": "${EXP_RESPONSES}[queries][${query_idx}][query_text]"}
