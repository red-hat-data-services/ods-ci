...
*** Test Cases ***
Verify Bias Metrics Available In CLI For Models Deployed Prior To Enabling Trusty Service For Admin User
    ...
    Wait Until Keyword Succeeds  5 min  10 sec  Verify Model Is Registered with TrustyAI Service
...
Verify Model Is Registered with TrustyAI Service
    ...
    ${podname}=    Get Pod Name   ${PRJ_TITLE}    label_selector=modelmesh-service=modelmesh-serving
...
Verify TrustyAI Metrics Exists In Observe Metrics
    ...
    [Arguments]        ${model_query}    ${retry_attempts}    ${user_type}=admin
    ${metrics_value} =    Run OpenShift Metrics Query    ${model_query}    ${retry_attempts}    ${user_type}
