...
Verify User Can Serve And Query A Model
    ...
    ...    namespace=${test_namespace}
    ${pod_name}=  Get POD Names    namespace=${test_namespace}    label_selector=serving.kserve.io/inferenceservice=${flan_model_name}
...
Verify User Can Deploy Multiple Models In The Same Namespace
    ...
    ...    namespace=${test_namespace}
    ${pod_name}=  Get POD Names    namespace=${test_namespace}    label_selector=serving.kserve.io/inferenceservice=${model_one_name}
    IF    ${IS_KSERVE_RAW}     Start Port-forwarding    namespace=${test_namespace}    pod_name=${pod_name}
    ...
    ${pod_name}=  Get POD Names    namespace=${test_namespace}    label_selector=serving.kserve.io/inferenceservice=${model_two_name}
    IF    ${IS_KSERVE_RAW}     Start Port-forwarding    namespace=${test_namespace}    pod_name=${pod_name}
...
Verify User Can Deploy Multiple Models In Different Namespaces
    ...
    ...    namespace=singlemodel-multi2
    ${pod_name}=  Get POD Names    namespace=singlemodel-multi1    label_selector=serving.kserve.io/inferenceservice=${model_one_name}
    IF    ${IS_KSERVE_RAW}     Start Port-forwarding    namespace=${test_namespace}    pod_name=${pod_name}
    ...    process_alias=llm-one
    Query Model Multiple Times    model_name=${model_one_name}    runtime=${TGIS_RUNTIME_NAME}
    ...    n_times=2    namespace=singlemodel-multi1    port_forwarding=${IS_KSERVE_RAW}
    IF    ${IS_KSERVE_RAW}    Terminate Process    llm-one    kill=true
    ${pod_name}=  Get POD Names    namespace=singlemodel-multi2    label_selector=serving.kserve.io/inferenceservice=${model_two_name}
    IF    ${IS_KSERVE_RAW}     Start Port-forwarding    namespace=${test_namespace}    pod_name=${pod_name}
    ...    process_alias=llm-two
    Query Model Multiple Times    model_name=${model_two_name}    runtime=${TGIS_RUNTIME_NAME}
...
Verify User Can Change The Minimum Number Of Replicas For A Model
    ...
    ${pod_name}=  Get POD Names    namespace=${test_namespace}    label_selector=serving.kserve.io/inferenceservice=${model_name}
    IF    ${IS_KSERVE_RAW}     Start Port-forwarding    namespace=${test_namespace}    pod_name=${pod_name}
