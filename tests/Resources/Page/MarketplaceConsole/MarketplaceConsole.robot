...
Get the OC commands For Cluster registration
    ...
    Wait Until Keyword Succeeds    30    1    Element Should Be Visible
    ...    //button[contains(@class, 'disabled')]/span[contains(text(),'Generate Secret')]
    sleep    5
    ${elem} =    Get WebElements    xpath://div[@role='textbox']//pre
    &{oc_command_dict} =    Create Dictionary
    @{itemname} =    Create List    Create namespace    Red Hat Marketplace Subscription
    ...    Red Hat Marketplace Kubernetes Secret    Red Hat Marketplace global pull secret
