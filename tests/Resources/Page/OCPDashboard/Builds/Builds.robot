...
Delete BuildConfig using Name
    ...
    Run Keyword If    '${status}'=='PASS'    OpenShiftCLI.Delete    kind=BuildConfig    name=${name}
    ...    namespace=${namespace}    ELSE    FAIL
    ...    No BuildConfig present with name '${name}' in '${namespace}' namespace, Check the BuildConfig name and namespace provide is correct and try again
...
Check If BuildConfig Exists
    [Arguments]    ${namespace}    ${name}
    ${status}    ${val}    Run keyword and Ignore Error    OpenShiftCLI.Get    kind=BuildConfig
    ...    namespace=${namespace}    field_selector=metadata.name==${name}
    [Return]    ${status}

Dependent Build should not Present
    [Arguments]    ${selector}
    ${isExist}    Run Keyword and Return Status    OpenShiftCLI.Get    kind=Build
    ...    label_selector=buildconfig=${selector}
