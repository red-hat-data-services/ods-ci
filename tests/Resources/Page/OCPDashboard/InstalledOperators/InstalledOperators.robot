...
Is Operator Installed
    [Arguments]    ${operator}
    Run Keyword and Return Status    Wait Until Element is Visible    //a[@data-test-operator-row="${operator}"]
    ...    timeout=10
...
Get RHODS version
    ...
    ${ver} =    Run
    ...    oc get csv -n redhat-ods-operator | grep "rhods-operator" | awk '{print $1}' | sed 's/rhods-operator.//'
