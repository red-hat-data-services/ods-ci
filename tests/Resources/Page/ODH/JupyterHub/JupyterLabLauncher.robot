...
JupyterLab Code Cell Error Output Should Not Be Visible
    ...
    ...    xpath://div[contains(@class,"jp-OutputArea-output") and @data-mime-type="application/vnd.jupyter.stderr"]
    ...    A JupyterLab code cell output returned an error

Get JupyterLab Code Cell Error Text
    ${error_txt} =    Get Text
    ...    //div[contains(@class,"jp-OutputArea-output") and @data-mime-type="application/vnd.jupyter.stderr"]
...
Maybe Select Kernel
    ${is_kernel_selected} =    Run Keyword And Return Status    Page Should Not Contain Element
    ...    xpath=//div[@class="jp-Dialog-buttonLabel"][.="Select"]
...
...    Note: this command requires ${admin_username}    to be logged to the cluster (oc login ...) and to have the user's notebook pod running (e.g. jupyterhub-nb-ldap-2duser1)
    ...
    IF    '${oc_whoami}' == '${admin_username}'
        ...
        ${output} =    Run
        ...    oc exec ${notebook_pod_name} -n rhods-notebooks -- find /opt/app-root/src/ -not -path '*/\.*' -not -path '/opt/app-root/src/' -exec rm -rv {} +
        Log    ${output}
    ELSE
...
JupyterLab Is Visible
    ${jupyterlab_visible} =    Run Keyword and Return Status    Wait Until Element Is Visible
    ...    xpath:${JL_TABBAR_CONTENT_XPATH}    timeout=30
...
Handle Kernel Restarts
    ...
    ${kernel_or_server_restarting} =    Run Keyword And Return Status    Page Should Not Contain Element
    ...    xpath:/html/body/div[8]/div/div[2]
    IF    ${kernel_or_server_restarting} == False
        ${is_server_down} =    Run Keyword And Return Status    Page Should Not Contain Element
        ...    xpath:/html/body/div[8]/div/div[2]/button[2]
...
Maybe Close Popup
    ...
    FOR    ${index}    IN RANGE    0    2+${len}
        ...
        ${accept} =    Get WebElements
        ...    xpath://div[contains(concat(' ',normalize-space(@class),' '),' jp-Dialog-footer ')]
        # Click the right most button of the popup
        Run Keyword If    ${accept}    Click Element
        ...    xpath://div[contains(concat(' ',normalize-space(@class),' '),' jp-Dialog-footer ')]/button[last()]
        Capture Page Screenshot
    END
...
Add and Run JupyterLab Code Cell in Active Notebook
    ...
    Sleep    0.1s
    Click Element
    ...    xpath://div[@aria-labelledby="${tab-id}"]//div[contains(concat(' ',normalize-space(@class),' '),' jp-mod-selected ')]
...
Wait Until JupyterLab Code Cell Is Not Active In a Given Tab
    ...
    Wait Until Element Is Not Visible
    ...    //div[@aria-labelledby="${tab_id_to_wait}"]/div[@aria-label="notebook content"]/div[1]/div[contains(@class, p-Cell-inputWrapper)]/div[contains(@class,"jp-Cell-inputArea")]/div[contains(@class,"jp-InputArea-prompt") and (.="[*]:")][1]
    ...    ${timeout}
...
Get JupyterLab Code Output In a Given Tab
    ...
    ${outputtext} =    Get Text
    ...    (//div[@aria-labelledby="${tab_id_to_read}"]/div[@aria-label="notebook content"]/div[1]/div[contains(@class, jp-Cell-outputWrapper)]/div[contains(@class,"jp-Cell-outputArea")]//div[contains(@class,"jp-RenderedText")])[last()]
    [Return]    ${outputtext}
...
Verify Installed Library Version
    [Arguments]    ${lib}    ${ver}
    ${status}    ${value} =    Run Keyword And Warn On Failure    Run Cell And Check Output
    ...    !pip show ${lib} | grep Version: | awk '{split($0,a); print a[2]}' | awk '{split($0,b,"."); printf "%s.%s", b[1], b[2]}'
    ...    ${ver}
