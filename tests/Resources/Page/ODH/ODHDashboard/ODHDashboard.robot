...
Launch ${dashboard_app} From RHODS Dashboard Link
    Wait Until RHODS Dashboard ${dashboard_app} Is Visible
    Click Link
    ...    xpath://div[@class="pf-c-card__title" and .="${dashboard_app}"]/../div[contains(@class,"pf-c-card__footer")]/a
    Switch Window    NEW

Launch ${dashboard_app} From RHODS Dashboard Dropdown
    Wait Until RHODS Dashboard ${dashboard_app} Is Visible
    Click Button
    ...    xpath://div[@class="pf-c-card__title" and .="${dashboard_app}"]/..//button[contains(@class,pf-c-dropdown__toggle)]
...
Verify Service Is Enabled
    ...
    ...    message=${app_name} should be enabled in ODS Dashboard
    Page Should Not Contain Element
    ...    xpath://article//*[.='${app_name}']/..//div[contains(@class,'enabled-controls')]/span[contains(@class,'disabled-text')]
    ...    message=${app_name} is marked as Disabled. Check the license
...
Remove Disabled Application From Enabled Page
    ...
    [Arguments]    ${app_id}
    ${card_disabled_xp}=    Set Variable
    ...    //article[@id='${app_id}']//div[contains(@class,'enabled-controls')]/span[contains(@class,'disabled-text')]
...
Verify Service Provides "Enable" Button In The Explore Page
    ...
    ...    error=${app_name} does not have sidebar with information in the Explore page of ODS Dashboard
    Page Should Contain Button    ${ODH_DASHBOARD_SIDEBAR_HEADER_ENABLE_BUTTON}
    ...    message=${app_name} does not have a "Enable" button in ODS Dashboard
...
Verify Service Provides "Get Started" Button In The Explore Page
    ...
    Wait Until Page Contains Element    ${ODH_DASHBOARD_SIDEBAR_HEADER_TITLE}    timeout=10
    ...    error=${app_name} does not have sidebar with information in the Explore page of ODS Dashboard
    Page Should Contain Element    ${ODH_DASHBOARD_SIDEBAR_HEADER_GET_STARTED_ELEMENT}
    ...    message=${app_name} does not have a "Get started" button in ODS Dashboard
...
Check Card Badges And Return Titles
    ...
    Run Keyword And Continue On Failure    Lists Should Be Equal    ${card_badges_titles}
    ...    ${expected_data}[${app_id}][badges]
    Run Keyword If    $RH_BADGE_TITLE in $card_badges_titles    Run Keyword And Continue On Failure
    ...    Page Should Contain Element    xpath:${card_locator}/${OFFICIAL_BADGE_XP}
...
Open Get Started Sidebar And Return Status
    ...
    ...    xpath://div[contains(@class,'pf-c-drawer__panel-main')]
    Sleep    1
    [Return]    ${status}
...
Check Sidebar Header Text
    ...
    ${getstarted_title}=    Get Text
    ...    xpath://div[contains(@class,'pf-c-drawer__panel-main')]//div[@class='odh-get-started__header']/h1[contains(@class, 'title')]
    ${getstarted_provider}=    Get Text
    ...    xpath://div[contains(@class,'pf-c-drawer__panel-main')]//div[@class='odh-get-started__header']//span[contains(@class, 'provider')]
    Run Keyword And Continue On Failure    Should Be Equal    ${getstarted_title}    ${expected_data}[${app_id}][title]
    Run Keyword And Continue On Failure    Should Be Equal    ${getstarted_provider}
    ...    ${expected_data}[${app_id}][provider]
...
Check Cards Details Are Correct
    ...
    FOR    ${idx}    IN RANGE    1    ${card_n}+1
        ...
        Check Get Started Sidebar    card_locator=${card_xp}    card_badges=${badges_titles}
        ...    app_id=${application_id}    expected_data=${expected_data}
    END
