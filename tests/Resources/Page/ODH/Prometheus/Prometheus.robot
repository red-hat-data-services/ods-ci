...
Run Query
    ...
    ${resp}=    RequestsLibrary.GET    url=${pm_url}/api/v1/query?query=${pm_query}    headers=${pm_headers}
    ...    verify=${False}
...
Get Rules
    ...
    ${pm_headers}=    Create Dictionary    Authorization=Bearer ${pm_token}
    ${resp}=    RequestsLibrary.GET    url=${pm_url}/api/v1/rules?type=${rule_type}    headers=${pm_headers}
    ...    verify=${False}
    Status Should Be    200    ${resp}
    [Return]    ${resp.json()}
