...
Verify Grafana Is Shipped And Enabled Within ODS
    ...
    Verify Resources Field Values    status.phase    Running    @{grafana_pods}
    Verify Resources Field Values    status.containerStatuses[0].name    auth-proxy    @{grafana_pods}
    Verify Resources Field Values    status.containerStatuses[1].name    grafana    @{grafana_pods}
    Verify Resources Field Values    status.conditions[2].status    True    @{grafana_pods}
    Verify Resources Field Values    status.replicas    2    @{grafana_deployment}
    Verify Resources Field Values    status.readyReplicas    2    @{grafana_deployment}
    Verify Resources Field Values    spec.ports[0].name    https    @{grafana_services}
    Verify Resources Field Values    spec.ports[0].port    443    @{grafana_services}
    Verify Resources Field Values    spec.ports[0].protocol    TCP    @{grafana_services}
    Verify Resources Field Values    spec.ports[0].targetPort    8443    @{grafana_services}
    Verify Resources Field Values Using RegExp    spec.clusterIP    ^((25[0-5]|(2[0-4]|1[0-9]|[1-9]|)[0-9])(\.(?!$)|$)){4}$
    ...    @{grafana_services}
    Verify Resources Field Values    status.readyReplicas    2    @{grafana_replicasets}
    Verify Resources Field Values    status.replicas    2    @{grafana_replicasets}
    Verify Resources Field Values    spec.port.targetPort    https    @{grafana_routes}
    Verify Resources Field Values    spec.to.name    grafana    @{grafana_routes}
    Verify Resources Field Values Using RegExp    spec.host    ^(grafana-redhat-ods-monitoring.*)    @{grafana_routes}
