...
Verify Odh-deployer Checks Cluster Platform Type
    ...
    ELSE IF    "${cluster_platform_type}" == "AWS"
        ${odhdeployer_logs_content}=     Set Variable    INFO: Deploying on AWS. Creating CRO for deployment of RDS Instance
    ELSE
        ${odhdeployer_logs_content}=     Set Variable    ERROR: Deploying on ${cluster_platform_type}, which is not supported. Failing Installation
    END
    ${odhdeployer_logs}=    Fetch Odh-deployer Pod Logs
    Pod Logs Should Contain    ${odhdeployer_logs}    ${odhdeployer_logs_content}
...
Fetch Odh-deployer Pod Logs
    ...
    ${odhdeployer_pod_logs}=    Oc Get Pod Logs  name=${odhdeployer_pod_info.metadata.name}  namespace=redhat-ods-operator  container=rhods-deployer
    [Return]    ${odhdeployer_pod_Logs}

Pod Logs Should Contain
    [Documentation]    Checks if the specified pod logs contain the specified content
    ...    Args:
    ...        pod_logs: Pod Logs
    ...        content: Content
    ...    Returns:
    ...        None
    [Arguments]    ${pod_Logs}    ${content}
    Should Contain    ${pod_logs}    ${content}
