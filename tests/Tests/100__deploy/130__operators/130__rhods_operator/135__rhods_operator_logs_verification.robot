...
Verify RHODS Operator log
    ...
    #Get the POD name
    ${data}    Run keyword    OpenShiftCLI.Get    kind=Pod    namespace=${namespace}
    ...
    Run Keyword IF    len($val)==${0} or "error:" in $val    FAIL
    ...    Either OC command has not been executed sucessfully or Logs is not present
