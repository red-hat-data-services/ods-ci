...
Test User Notebook Plugin in JupyterLab
    ...
    Run Keyword IF    ${image_mismatch_plugins} != &{EMPTY}    Fail
    ...    Plugin mismatch Found in the mentioned images '${image_mismatch_plugins}'    ELSE    Log To Console
    ...    All the plugin is matched between the old and new notebook images
...
Gather Notebook data
    ${notebook_data}    Create Dictionary    s2i-minimal-notebook=${s2i-minimal-notebook}
    ...    s2i-generic-data-science-notebook=${s2i-generic-data-science-notebook}    tensorflow=${tensorflow}
    ...    pytorch=${pytorch}
...
Verify the Plugins for each JL images
    FOR    ${image}    IN    @{notebook_images}
        ...
        IF    len(${plugin_names}) >= len(${old_notebok_plugin})
            ...
            END
            Run Keyword IF    ${mistamtch_plugins} != @{EMPTY}    Set To Dictionary    ${image_mismatch_plugins}
            ...    ${image}    ${mistamtch_plugins}
        ELSE
            ...
            ...    Plugins '${missing_plugins}' has been removed from the '${image}' notebook image
        END
