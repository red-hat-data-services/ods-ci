...
Verify Anaconda Commercial Edition Is Available In RHODS Dashboard Explore/Enabled Page
    ...
    # robocop: disable
    Run Keyword If    ${status} == ${False}    Run Keywords    Verify Service Is Enabled    Anaconda Commercial Edition
    ...    AND    FAIL    Anaconda Commercial Edition does not have a "Enable" button
    ...    in ODH Dashboard since it has been alreday Enabled and Present in Enabled Page
...
Verify Anaconda Commercial Edition Fails Activation When Key Is Invalid
    ...
    [Tags]    Smoke    Sanity    ODS-310    ODS-367
    Open Browser    ${ODH_DASHBOARD_URL}    browser=${BROWSER.NAME}    options=${BROWSER.OPTIONS}
    Login To RHODS Dashboard    ${TEST_USER.USERNAME}    ${TEST_USER.PASSWORD}    ${TEST_USER.AUTH_TYPE}
    Wait For RHODS Dashboard To Load
...
Verify User Is Able to Activate Anaconda Commercial Edition
    ...
    [Tags]    Sanity    Smoke    ODS-272    ODS-344    ODS-501
    Open Browser    ${ODH_DASHBOARD_URL}    browser=${BROWSER.NAME}    options=${BROWSER.OPTIONS}
    Login To RHODS Dashboard    ${TEST_USER.USERNAME}    ${TEST_USER.PASSWORD}    ${TEST_USER.AUTH_TYPE}
    Wait For RHODS Dashboard To Load
