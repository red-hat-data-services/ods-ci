...
class OpenshiftOps():
    ...
    def openshift_install(self, config_file="ocp_config_file.yaml"):
        ...
        process = subprocess.Popen(['openshift-install', 'create', 'cluster'],
        ...
        match = re.search('.*Access the OpenShift web-console here: (\S+).*Login to the console with user: "(\S+)".*password: "(\S+)".*', output, re.S)
        if match is None:
...
if __name__ == "__main__":
    ...
    required_openshift_install_parser.add_argument("--aws-accesskey-id",
                                                help="aws access key id",
                                                action="store", dest="aws_access_key_id",
                                                required=True)

    required_openshift_install_parser.add_argument("--aws-secret-accesskey",
                                                help="aws secret access key",
                                                action="store", dest="aws_secret_access_key",
                                                required=True)

    required_openshift_install_parser.add_argument("--install-config-file",
                                                help="Install config file. Note: Place this file from where you are running this for now",
                                                action="store", dest="install_config_file",
                                                required=True)

    optional_openshift_install_parser.add_argument("--aws-region",
                                                help="aws region",
                                                action="store", dest="aws_region", metavar="",
                                                default="us-east-1")

    optional_openshift_install_parser.add_argument("--cluster-name",
    ...
    openshift_destroy_parser.add_argument("--install-config-file",
